...
class ApiNewsWebview extends Controller
{
    ...
    public function detailNews(Request $request)
    {
        ...
        if ($news) {
            $news['news_image_dalam'] = env('STORAGE_URL_API').$news['news_image_dalam'];
            ...
            if (!empty($news['news_product'])) {
                ...
                foreach ($newsProduct as $keyProduct => $valProduct) {
                    ...
                    $news['news_product'][$keyProduct]['product_image'] = env('STORAGE_URL_API').($valProduct['product']['photos'][0]['product_photo']??'img/product/item/default.png');
                }
            }

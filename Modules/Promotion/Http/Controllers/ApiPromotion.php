...
class ApiPromotion extends Controller
{
	...
	public function update(Request $request){
		...
		if(isset($post['promotion_series_days'])){
			foreach($post['promotion_series_days'] as $key => $row){
				...
				$data['voucher_value']			= $post['voucher_value'][$key];
							...
							if(count($contentOld) > 0){
								foreach($contentOld as $old){
									if($old['content_type'] == 'image' || $old['content_type'] == 'file'){
										MyHelper::deletePhoto(str_replace(env('APP_API_URL'), '', $old['content']));
									}
								}
							...
							if($content['content']){
								...
								if($content['id_whatsapp_content']){
									...
									if($waContent && ($waContent->content_type == 'image' || $waContent->content_type == 'file')){
										MyHelper::deletePhoto(str_replace(env('APP_API_URL'), '', $waContent->content));
									}
								}
				...
				if($query){
					...
					foreach($contentWhatsapp as $waContent){
						...
						if(!$insertWhatsapp){
							...
							$result = [
								'status'	=> 'fail',
								'messages'	=> ['Update WhatsApp Content Failed.2']
							];
							return response()->json($result);
						}
					}
				...
				if(isset($post['promotion_channel'][$key]) && in_array('deals', $post['promotion_channel'][$key])){
					//kalo ada deals
					$dataDeals 							= [];
					$dataDeals['deals_type']			= "Hidden";
					$dataDeals['deals_promo_id_type']	= $post['deals_promo_id_type'][$key];
					...
					if($post['deals_promo_id_type'][$key] == 'promoid'){
						$dataDeals['deals_promo_id']	= $post['deals_promo_id'][$key];
					} else {
						$dataDeals['deals_promo_id']	= $post['deals_nominal'][$key];
					}
					
					$dataDeals['deals_title'] = "[Promotion] ".$promotion['promotion_name'];
					$dataDeals['deals_second_title'] = "Campaign No ".$key;
					$dataDeals['deals_description'] = "This hidden deals is auto-generated from Promotion '".$promotion['promotion_name']."'";
					$dataDeals['deals_short_description'] = "This hidden deals is auto-generated from Promotion '".$promotion['promotion_name']."'";
					$dataDeals['deals_image'] = "";
					$dataDeals['deals_start'] = date('Y-m-d 00:00:00', strtotime($post['deals_start'][$key]));
					$dataDeals['deals_end'] = date('Y-m-d 00:00:00', strtotime($post['deals_end'][$key]));
					if($post['duration'][$key] == "duration"){
						...
						$dataDeals['deals_voucher_expired'] = date('Y-m-d 00:00:00', strtotime($post['deals_voucher_expiry_bydate'][$key]));
					}
			...
			$data['voucher_value']			= $post['voucher_value'][0];
			...
			if(isset($post['promotion_channel'][0]) && in_array('deals', $post['promotion_channel'][0])){
				...
				$dataDeals 							= [];
				$dataDeals['deals_type']			= "Hidden";
				$dataDeals['deals_promo_id_type']	= $post['deals_promo_id_type'][0];
				...
				if($post['deals_promo_id_type'][0] == 'promoid'){
					$dataDeals['deals_promo_id']	= $post['deals_promo_id'][0];
				} else {
					$dataDeals['deals_promo_id']	= $post['deals_nominal'][0];
				}
				
				$dataDeals['deals_title'] = "[Promotion] ".$promotion['promotion_name'];
				$dataDeals['deals_second_title'] = "Campaign";
				$dataDeals['deals_description'] = "This hidden deals is auto-generated from Promotion '".$promotion['promotion_name']."'";
				$dataDeals['deals_short_description'] = "This hidden deals is auto-generated from Promotion '".$promotion['promotion_name']."'";
				$dataDeals['deals_image'] = "";
				$dataDeals['deals_start'] = date('Y-m-d 00:00:00', strtotime($post['deals_start'][0]));
				$dataDeals['deals_end'] = date('Y-m-d 00:00:00', strtotime($post['deals_end'][0]));
				if($post['duration'][0] == "duration"){
					$dataDeals['deals_voucher_duration'] = $post['deals_voucher_expiry_duration'][0];
					$dataDeals['deals_voucher_expired'] = null;
				}
				else {
					$dataDeals['deals_voucher_duration'] = null;
					$dataDeals['deals_voucher_expired'] = date('Y-m-d 00:00:00', strtotime($post['deals_voucher_expiry_bydate'][0]));
				}
				...
				if($dealsQuery){
					...
					if($post['id_outlet'][0] != ""){
						$deleteDealsOutlet =  DealsOutlet::where('id_deals', $id_deals)->delete();
						if (in_array("all", $post['id_outlet'][0])) {
							/* SELECT ALL OUTLET */
	...
	public function sendPromotion(Request $request)
    {
		...
		foreach($queue as $key => $dataQueue){
			...
			if(isset($idDealsVoucher)){
				$dataPromotionSent['id_deals_voucher'] 	= implode(',',$idDealsVoucher);
			}
	...
	public function promotionVoucherList(Request $request)
	{
		...
		$voucher = $voucher->leftJoin('users', 'users.id', 'deals_users.id_user')
							->where('id_deals', $promotionContent->id_deals)
							->paginate(10)->toArray();
